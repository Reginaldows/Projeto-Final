...
try {
    ...
    $stmt = $conexao->prepare(
        "SELECT id, nome, email FROM dados WHERE REPLACE(REPLACE(REPLACE(cpf, '.', ''), '-', ''), ' ', '') = ?"
    );
    if (!$stmt) {
    ...
    try {
        ...
        enviarResposta(500, ['erro' => 'Erro ao enviar email. Tente novamente mais tarde.']);
    }
} catch (Exception $e) {
    enviarResposta(500, ['erro' => 'Erro interno do servidor']);
}
